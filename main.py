from config import user_token, group_token, offset
import vk_api
import requests
import datetime
...
class VKinder:
    def __init__(self):
        ...
        print("Бот запущен, перейдите в группу VK")
    ...
    def get_age_low(self, user_id):
        ...
        try:
            ...
            elif len(date_list) == 2 or date not in dictionary:
    ...
    def get_age_high(self, user_id):
        ...
        try:
            ...
            if len(date_list) == 3:
                ...
                year_now = int(datetime.date.today().year)
                return year_now - year
            elif len(date_list) == 2 or date_ not in dictionary:
    ...
    def find_persons(self, user_id, offset):
        ...
        self.send_photo_1(user_id, 'Фотография № 1', offset)
        if self.get_photo_2(self.person_id(offset)) != None:
            self.send_photo_2(user_id, 'Фотография № 2', offset)
            self.send_photo_3(user_id, 'Фотография № 3', offset)
        else:
            self.write_msg(user_id, f'Это все фотографии')
    ...
    def found_person_info(self, offset):
        ...
        return f'{list_person[0]} {list_person[1]}, ссылка на страницу VK - {list_person[3]}'
